<?php
	...
	// ACCIO PATCH — SVG-only icons
	//
	// Upstream ships one favicon.ico, 20 app-icon PNGs, 8 touch-icon PNGs and
	// 4 tile PNGs, every one of them a raster of the MWC mark. We replace the
	// four SVG marks (images/logo_big.svg, logo_small.svg, app_icons/
	// app_icon.svg, mask_images/mask_image.svg) and empty the raster lists,
	// rather than commit 33 hand-made PNGs: the build is offline and
	// deterministic by design, so it cannot rasterise at build time, and
	// leaving the lists populated would ship the MWC bowtie as our favicon —
	// exactly the branding leak this packet exists to close.
	//
	// SVG-only is also this repo's house convention: 051 Fidelius, 06b
	// GrinScan, 06d Tiny Explorer and the 07 pool all ship an SVG favicon and
	// no raster set.
	//
	// The PNG FILES stay in the tree (unreferenced, ~100 KB); deleting them is
	// S9. The one real loss is the iOS home-screen touch icon, which has no
	// SVG form — see script052_implementation.md, S5 residuals, for the
	// recipe to add it later by dropping PNGs into patches/ with no code
	// change.
	...
	// App icons — ACCIO PATCH: the SVG mark only, see the note above
	const APP_ICONS = [
	...
	// Touch icons — ACCIO PATCH: emptied, see the note above. iOS has no SVG
	// touch icon, so a home-screen bookmark falls back to a screenshot until
	// the PNG set is generated.
	const TOUCH_ICONS = [
	];
	...
	// Tile images — ACCIO PATCH: emptied. These are Windows 8 / IE11 start
	// tiles; no shipping browser reads browserconfig.xml any more.
	const TILE_IMAGES = [
	];
	...
	$files = [
		...
		// ACCIO PATCH — the licence text the MWC Wallet credit in ATTRIBUTIONS
		// links to. Listed here for the same reason every other "<name>
		// license.txt" is: entries in this table are what scripts/
		// service_worker.js precaches, and a credit whose licence link 404s
		// once the PWA is offline is not much of a credit.
		//
		// Checksum NULL because nothing fetches it with an integrity
		// attribute — it is an <a href>, exactly like upstream's own licence
		// links. Version 0 means "emit no ?query".
		"./MWC Wallet license.txt" => [
			"Version" => 0,
			"Cache" => TRUE,
			"Minified" => FALSE,
			"Checksum" => NULL
	...
	const ATTRIBUTIONS = [
		...
		// ACCIO PATCH — the credit.
		//
		// Accio is a fork of MWC Wallet by Nicolas Flamel, vendored under MIT
		// at adef11daadf04e4ec259d6febcc720da3daaa6c7 (v2.8.2). MIT requires
		// the copyright notice be retained; this entry is the courteous
		// version of that obligation, in the app's own idiom, and it is the
		// ONE place "MWC" is meant to survive in the built output.
		//
		// Rendered by scripts/about_section.js, which passes the name and both
		// link targets as translation ARGUMENTS — and scripts/language.js
		// brands arguments by whole-value match only, so this name is never
		// rewritten by the brand layer that renames everything else.
		//
		// The upstream copyright line in the About section's Copyright block
		// ("© 2022–… Nicolas Flamel.") is upstream's and stays verbatim; it is
		// the licence condition under which we may use the code at all.
		"MWC Wallet" => [
			"URL" => "https://github.com/NicolasFlamel1/mwcwallet.com",
			"License Path" => "./MWC Wallet license.txt",
			"License Type" => "MIT License"
		],
		"Noto" => [
