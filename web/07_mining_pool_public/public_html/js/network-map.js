 ...
 * miners is real data (hub + gateway·offline + node twinkle), not "empty". There is NO
 * sample/demo dataset: when a feed is unreachable (the operator has not published it, or
 * the API is down) the globe renders bare and the on-page note says so. A fabricated
 * topology on a live pool read as real data (2026-09-13), so it was removed.
...
(function () {
  ...
  function ingest(topo, peers) {
    ...
    if (pts.length) {
      ...
      const want = Math.min(140, Math.max(0, total));
      let made = 0, guard = 0;
  ...
  function buildDonut(svgId, legendId, raw, centerLabel) {
    ...
    const reset=()=>{ num.textContent=sum; lbl.textContent=centerLabel; slices.forEach(s=>{ s.path.style.opacity=1; s.row.classList.remove("hot"); }); };
  ...
  async function boot() {
    ...
    let topo = null, peers = null, topoOk = false, peersOk = false;
    try { topo = await jget('/api/pool/topology'); topoOk = !!topo; } catch (_) {}
    try { peers = await jget('/api/network/peers?window=30d'); peersOk = !!peers; } catch (_) {}
    ...
    // twinkle), NOT "empty". An UNREACHABLE feed (publishing switched off → 404, or the API
    // down) renders a BARE globe with zeroed placards and a note — never a stand-in dataset.
    // A sample topology used to fill this gap and was read as the pool's real footprint the
    // first time anyone looked (2026-07-26, and again 2026-09-13): the note under the globe
    // is the only thing that says "sample", and nobody reads a 12px mono line before the
    // picture. `noMiners` is the softer state: the topology is real, there just aren't any
    // miner-countries online yet.
    ...
    // Why the peer layer is empty decides what the note may promise. `geo_available` is the
    // server saying whether the collector can work at all (geoip-lite installed); without it
    ...
    if (noteEl) {
      const msgs = [];
      if (!topoUsable) {
        msgs.push('Live network data is <b>not available</b> — this pool has not published its network map, or the feed is temporarily down');
      } else if (noMiners) {
      ...
      // Peer layer: one sentence per cause, and none when the topology sentence already covers
      // it (both feeds sit behind the same publish switch, so two "unavailable" lines would
      // describe one fact twice).
      if (!peersOk) {
        if (topoUsable) msgs.push('The Grin node sighting feed is temporarily unavailable');
      } else if (peersGeoOff) {
        msgs.push('Grin node sightings need the optional <code>geoip-lite</code> package on the pool node — the peer collector is idle without it');
      } else if (peersEmpty) {
        msgs.push('No Grin node sightings recorded yet — the peer collector takes its first snapshot shortly after the pool starts, then every 20 minutes');
      }
