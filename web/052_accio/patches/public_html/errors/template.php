...
<head>
	...
	<script>
		...
		function isApp() {
			...
			// Return if app
			return isMobileApp() === true || (isExtension() === false && ((typeof navigator === "object" && navigator !== null && "standalone" in navigator === true && navigator["standalone"] === true) || (typeof matchMedia === "function" && (matchMedia("(display-mode: standalone)")["matches"] === true || matchMedia("(display-mode: window-controls-overlay)")["matches"] === true))));
		}
	...
	<style>
		...
		* {
		...
		@media only screen and (display-mode: window-controls-overlay) {
			...
			body:not(.languagesAvailable) > span > span {
				width: 0 !important;
				padding: 0 !important;
				margin: 0 !important;
			}
		...
		main {
			height: 100%;
			width: 100%;
			width: calc(100% - var(--safe-area-inset-right) - var(--safe-area-inset-left));
			overflow: hidden;
			display: flex;
			flex-direction: column;
			align-items: center;
		...
		aside.message > div {
			...
			margin: auto;
			padding: 0 2em;
			padding: var(--safe-area-inset-top) calc(2em + var(--safe-area-inset-right)) var(--safe-area-inset-bottom) calc(2em + var(--safe-area-inset-left));
			display: table-cell;
...
<body class="loading" spellcheck="false"><!--
	...
	--><?php /* ACCIO PATCH — the ONLY change in this file: "Orbital Dawn" is
	          linked last here too, so the eight error pages this template
	          renders are the same wallet. They carry their own copy of the
	          ground, the spinner and the whole message dialog in the inline
	          <style> above, and nginx serves 404 and — during maintenance —
	          503 from here, so without this line the theme stops at the app.
	          The leading "." is upstream's convention in this file, and every
	          link above it does the same: errors/ is one directory down, so
	          "." . "./styles/accio.css" resolves to "../styles/accio.css". */ ?><!--
	--><link rel="stylesheet" type="text/css" href=".<?= encodeString(getResource("./styles/accio.css")); ?>" integrity="<?= encodeString(getChecksum("./styles/accio.css")); ?>"><!--
