...
| Grin Transporter payout rail | ❌ | no pool-side code at all — the dead `lib/wallet-transporter.js` stub was deleted 2026-08-06; only the reserved `incentives.transporter_enabled` key + disabled admin checkbox remain ([093 Transporter](script09_design.md)) |
...
| D4 | **Grin Transporter payout rail** | No pool-side code. The `lib/wallet-transporter.js` stub was deleted 2026-08-06 — nothing imported it, and the interface note it carried now lives in [script09_design.md](script09_design.md) B.9 #6. Blocked on the same gate: no mainstream wallet can receive from a relay. | Leave `incentives.transporter_enabled` reserved-and-false. **Do not remove that key without also removing the disabled checkbox in `settings-incentives.html`** — the settings harvester sends every `id` in `.settings-form` (disabled inputs included) and `updateSection` throws on an unknown key, so removing one alone breaks every Incentives save. |
