...
class Consensus {
	...
	// Public
		...
		static getWalletType() {
			...
			// ACCIO PATCH — Accio is a Grin wallet, and only a Grin wallet.
			//
			// Answered before ANY of the resolution below, so local storage,
			// the "Override Wallet Type" URL parameter and a stale saved value
			// from an earlier visit can none of them select another chain. The
			// wallet type is a consensus discriminator — HRP, emission, address
			// type — so a caller that gets MWC here does not merely look wrong,
			// it builds transactions for a different chain.
			//
			// Note what this deliberately does NOT do: it does not rename or
			// remove Consensus.MWC_WALLET_TYPE. Those code paths stay intact
			// and simply become unreachable, so `git log <PIN>..upstream` stays
			// readable and the Grin path can never be confused with a
			// mislabelled MWC one. Deleting them is S9+, one subsystem at a
			// time (design §"Naming policy" layer 3).
			// Set consensus's wallet type, which web workers and anything
			// reading the cached property still expect to be populated
		...
		// Upstream's wallet type resolution, kept verbatim but unreachable.
		// Not deleted: keeping it here is what makes the diff against upstream
		// one added block instead of a 100-line rewrite, so a future upstream
		// change to this logic is still reviewable.
		static getWalletTypeUpstream() {
	...
	// Private
		...
		static get DEFAULT_WALLET_TYPE() {
			...
			// ACCIO PATCH — Grin, not MWC. getWalletType() above already makes
			// this unreachable for its own resolution, but the constant is read
			// elsewhere and a wrong value here would be a silent chain mismatch.
