...
class CheckForUpdates {
	...
	// Public
		...
		constructor(unlocked, settings, cookieAcceptance) {
			...
			// ACCIO PATCH — the update check points at our repository, and the
			// standalone build no longer performs it.
			//
			// THIS FILE HAD NO MARKER UNTIL THE R8 REVIEW. Six URLs were
			// rewritten in S5 with nothing to grep for, so a review following
			// the documented method — grep the marker, never diff — read none
			// of them. That is the finding; the marker above is the fix.
			//
			// Two things below:
			//
			// 1. The three upstream repositories become ours. Accio builds
			//    neither an extension nor a mobile app, so only the third
			//    branch was ever reachable — and it is the one now removed.
			//    The other two are left pointing somewhere honest rather than
			//    at NicolasFlamel1, in case a later packet builds them.
			//
			// 2. The `location["protocol"] === Common.FILE_PROTOCOL` branch is
			//    GONE, so the standalone artefact falls through to the same
			//    `else` the hosted site takes: no URL, no request, and the
			//    setting hidden. It used to fetch
			//    api.github.com/repos/…/releases/latest on load, with the
			//    setting defaulting to true — an unsolicited third-party
			//    request from an artefact whose whole purpose is being usable
			//    offline, disclosing the visitor's IP and the fact that they
			//    run a Grin wallet, and undisclosed in the privacy policy we
			//    ship beside it.
			//
			//    It could not have worked in any case: it compares the
			//    toolkit's own `v*` release tags against VERSION_NUMBER, which
			//    is upstream's wallet version ("2.8.2" in backend/common.php).
			//    Two unrelated version lines, compared with >=, which fires on
			//    equal — so the prompt is noise whichever way it lands.
			//
			// Restoring it is one `else if` block; a version line the two
			// halves share would have to come with it.
			...
			if(Common.isExtension() === true && (typeof browser === "undefined" || browser["runtime"]["id"] !== "{d783f67c-4dea-4d64-bfc2-1d4a6057babe}") && (typeof chrome === "undefined" || chrome["runtime"]["id"] !== "ahhdnimkkpkmclgcnbchlgijhmieongp") && (typeof chrome === "undefined" || chrome["runtime"]["id"] !== "nhfjkjhonhcjengnbaplabipoajnlmjm") && (typeof chrome === "undefined" || chrome["runtime"]["id"] !== "iaagomomhleebihpbgbnlbbfaeomeoof")) {
				...
				this.newestVersionUrl = "https://api.github.com/repos/noobvie/Grin-Node-Toolkit/releases/latest";
				
				// Set download URL
				this.downloadUrl = "https://github.com/noobvie/Grin-Node-Toolkit/releases";
			}
			...
			// Otherwise — including loading from a file, whose branch was
			// removed above
			else {
