...
RATELIMIT
    ...
    # vhost — HTTP only; step 4 obtains the cert and writes the full HTTPS config
...
ww_setup_ssl() {
    ...
    # ── Step 1: Obtain cert via webroot (nginx stays running) ─────────────────
    info "Requesting SSL certificate for $WW_DOMAIN ..."
    certbot certonly --webroot -w /var/www/letsencrypt \
        -d "$WW_DOMAIN" --non-interactive --agree-tos -m "$WW_EMAIL" \
...
faucet_setup_nginx() {
    ...
    # ── Step 1: Write HTTP-only config so nginx can serve the ACME challenge ───
    info "Writing temporary HTTP nginx config → $FAUCET_NGINX_CONF"
    cat > "$FAUCET_NGINX_CONF" << NGINX_HTTP
# Grin Testnet Faucet — temporary HTTP config for ACME challenge
server {
    listen 80;
    listen [::]:80;
    server_name $domain;
...
NGINX_HTTP
    ...
    nginx -t && systemctl reload nginx \
        || { warn "nginx config test failed — check $FAUCET_NGINX_CONF"; return; }

    # ── Step 2: Obtain SSL certificate via webroot (nginx stays running) ───────
    echo ""
