...
//
// Reserved stub for the planned self-hosted store-and-forward slate relay (Script 093,
...
// script09_design.md): does the wallet the miner already runs support receiving on a relay?
...
class WalletTransporter {
  constructor(config) {
    this.config = config || {};
    this.available = false; // never advertise as a usable rail until Script 093 lands
  }
  ...
  async send(/* grinAddress, amount */) {
    throw new Error('Grin Transporter payout rail is not implemented yet (Script 093). ' +
