...
class Language {
	...
	// Public
		...
		//
		// Client-side twin of brandText() in backend/language.php. The two
		// maps MUST stay identical: the server brands the translation table
		// (keys and values) that scripts/languages.js ships to this file, so a
		// key branded on one side and not the other simply stops matching and
		// the string silently falls back to English.
		//
		// PHRASES only, never the bare token "MWC" — that is the ticker and a
		// protocol discriminator (design §"Naming policy" layer 3, deleted in
		// S9+, never renamed).
		static brandText(text) {
			...
			// Go through all brand substitutions
			for(var i = 0; i < Language.BRAND_SUBSTITUTIONS["length"]; ++i)

				// Apply the substitution to the text
				text = text.split(Language.BRAND_SUBSTITUTIONS[i][0]).join(Language.BRAND_SUBSTITUTIONS[i][1]);

			// Return text
			return text;
		}

		// ACCIO PATCH — rebrand a translation argument
		//
		// Arguments are WHOLE-VALUE matched, never substring-replaced, because
		// this list also carries user data — wallet names, addresses, file
		// names. A substring rule here would silently rewrite a user's own
		// text; an exact match cannot, since no user value equals one of
		// upstream's URLs by accident.
		static brandArgument(value) {
			...
			// Check if the brand links haven't been resolved yet
			if(typeof Language.brandLinks === "undefined") {

				// Get this deployment's own addresses
				var site = (typeof HTTPS_SERVER_ADDRESS !== "undefined") ? HTTPS_SERVER_ADDRESS : location["origin"];
				var onion = (typeof TOR_SERVER_ADDRESS !== "undefined") ? TOR_SERVER_ADDRESS : site;

				// Set the brand links
				Language.brandLinks = {

					// Upstream's official site — this one is a phishing guard,
					// not decoration: it is the address the wallet tells a user
					// to verify against before they type a passphrase into it.
					"https://mwcwallet.com": site,
					"http://mwcwalletmiq3gdkmfbqlytxunvlxyli4m6zrqozk7xjc353ewqb6bad.onion": onion,

					// The "download it for free from …" release links in the
					// same warning. Only the /releases forms belong here: the
					// bare repository URLs are the About section's source-code
					// link and its translation contact, and those are set
					// explicitly in scripts/about_section.js instead.
					//
					// The bare URL https://github.com/NicolasFlamel1/mwcwallet.com
					// must NOT be remapped — it is the target of the MWC Wallet
					// credit in ATTRIBUTIONS, and rewriting it would point our
					// one attribution link at ourselves.
					"https://github.com/NicolasFlamel1/MWC-Wallet-Standalone/releases": Language.SOURCE_CODE_URL + "/releases",
					"https://github.com/NicolasFlamel1/MWC-Wallet-Browser-Extension/releases": Language.SOURCE_CODE_URL + "/releases",
					"https://github.com/NicolasFlamel1/MWC-Wallet-Mobile-App/releases": Language.SOURCE_CODE_URL + "/releases"
				};
			}

			// Return the rebranded value if one exists, otherwise the value
			return (Object.prototype.hasOwnProperty.call(Language.brandLinks, value) === true) ? Language.brandLinks[value] : value;
		}

		// ACCIO PATCH — rebrand a translation's arguments in place
		//
		// In place, because getTranslation mutates textArguments itself when
		// it truncates a number's fractional part. Replacing the array would
		// throw that mutation away.
		static brandArguments(textArguments) {

			// Check if text arguments isn't an array
			if(Array.isArray(textArguments) === false)

				// Return text arguments
				return textArguments;

			// Go through all text arguments
			for(var i = 0; i < textArguments["length"]; ++i) {

				// Check if text argument is a link's parts
				if(Array.isArray(textArguments[i]) === true) {

					// Go through all of the link's parts
					for(var j = 0; j < textArguments[i]["length"]; ++j)

						// Rebrand the part
						textArguments[i][j] = Language.brandArgument(textArguments[i][j]);
				}

				// Otherwise
				else

					// Rebrand the text argument
					textArguments[i] = Language.brandArgument(textArguments[i]);
			}
		...
		static getTranslation(text, textArguments = [], encodeHtml = false) {
			...
			// ACCIO PATCH — rebrand the lookup key and the arguments
			text = Language.brandText(text);
			Language.brandArguments(textArguments);
		...
		static getDefaultTranslation(text) {
			...
			// ACCIO PATCH — this becomes a data-text attribute, i.e. the key
			// the translator looks up on the next language change
			return Language.brandText(text);
		}

		// Create translatable container
		static createTranslatableContainer(type, text, textArguments = [], classes = "", textIsTitle = false) {

			// ACCIO PATCH — brand before the raw values are frozen into the
			// data-text / data-arguments attributes below, not only inside
			// getTranslation. Otherwise upstream's URLs survive in the DOM.
			text = Language.brandText(text);
			Language.brandArguments(textArguments);
	...
	// Private
		...
		//
		// Mirror of BRAND_SUBSTITUTIONS in backend/language.php. Ordered
		// pairs, applied in order. Keep the two files in step.
		static get BRAND_SUBSTITUTIONS() {

			// Return brand substitutions
			return [
				["MWC Wallet", "Accio"],
				["MimbleWimble Coin", "Grin"],

				// Upstream invites translators by email; we have no mailbox
				// for that, so the same slot becomes our issue tracker. Only
				// the English phrasing matches — other languages keep their
				// own wording around the (rebranded) link.
				["You can email %1$m", "You can open an issue at %1$m"],

				// ── The translated spellings of the same two brands ──────
				//
				// Added by the R8 review; see the long note beside the
				// mirror of this list in backend/language.php. Three of the
				// six translators did not keep the English word order, so 27
				// strings shipped unbranded — including the page <title>, the
				// error titles and the currency label beside a balance.
				// Measured before and after: 27 hits, then 0.
		...
		// ACCIO PATCH — Accio's source code
		static get SOURCE_CODE_URL() {

			// Return source code URL
			return "https://github.com/noobvie/Grin-Node-Toolkit";
		}

		// ACCIO PATCH — where to reach the people who run this deployment
		static get CONTACT_URL() {

			// Return contact URL
			return Language.SOURCE_CODE_URL + "/issues";
		}
