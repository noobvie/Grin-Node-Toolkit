...
(function () {
  ...
  // Escape before any interpolation into innerHTML. Node names are operator-authored region
  // labels from pool_locations (POST /api/admin/locations), i.e. free text that reaches this
  // page through the API — not an HTML-authoring field like the CMS body or an ad's html_code.
  // See audit §I10.
  ...
  function buildDonut(svgId, legendId, raw, centerLabel) {
    ...
    rows.forEach(([name,val]) => { const a0=ang, a1=ang+(val/total)*Math.PI*2; ang=a1; const col=colorFor(name);
      ...
      row.innerHTML='<span class="sw" style="background:'+col+'"></span><span class="nm">'+esc(name)+'</span><span class="pc">'+(val/total*100).toFixed(1)+'%</span>'; legend.appendChild(row); slices.push({path,row,name,val}); });
