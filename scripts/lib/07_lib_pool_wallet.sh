...
pw_write_launchers() {
    ...
    #    De-root (design §13.9): when the grinpool service user exists (pool hub),
    #    drop to it so every wallet file (lmdb, tx log, tor send dirs) stays
    #    grinpool-owned — the de-rooted backend spawns `grin-wallet send` as
    #    grinpool and must be able to open the same wallet db. The chown reclaims
    #    root-owned leftovers from init/address one-shots (idempotent, mirrors the
    #    node launch contract). HOME=wallet dir for the same reason as the node's.
    cat > "$lf" <<EOF
#!/bin/bash
# GENERATED by 07_lib_pool_wallet.sh — combined Owner+Foreign listener (port $PW_OWNER_PORT, no passphrase in argv).
cd "$dir" || exit 1
if [[ \$(id -u) -eq 0 ]] && id grinpool &>/dev/null; then
    chown -R grinpool:grinpool "$dir" 2>/dev/null || true
    exec su -s /bin/bash grinpool -c "cd '$dir' && HOME='$dir' exec '$bin' $PW_NET_FLAG --top_level_dir '$dir' owner_api"
fi
...
# ─── Replace the pool wallet (design §13.10e — compromise / corruption runbook) ─
# Safe by architecture: miner balances/owed amounts live in pool.db, NOT the
# wallet, so swapping wallets never loses accounting. This wrapper prints the
# runbook, then drives the EXISTING pw_* code: stop listener → pw_setup (operator
# picks "Create a NEW wallet" — the old dir is ARCHIVED aside, never deleted;
# pw_setup re-does toml patches, .wallet_pass, node wallet_listener_url re-patch,
# unlock, address record) → sweep guidance.
pw_replace_wallet() {
    echo -e "\n${BOLD:-}Replace pool wallet — ${_PW_NET}${RESET:-}"
    echo -e "  ${YELLOW:-}Use when the hot wallet is COMPROMISED or its data is corrupt.${RESET:-}"
    echo -e "  ${DIM:-}Miner balances live in pool.db — a wallet swap never loses accounting.${RESET:-}"
    echo ""
    echo -e "  ${BOLD:-}What will happen (existing setup steps, reused):${RESET:-}"
    echo -e "    1. YOU FIRST: pause payouts in the admin panel (Settings → withdrawals)."
    echo -e "    2. The running listener is stopped."
    echo -e "    3. Setup wallet runs — choose ${BOLD:-}2) Create a NEW wallet${RESET:-}: the old dir is"
    echo -e "       ARCHIVED aside (renamed, never deleted), a fresh seed is created"
    echo -e "       (write it down offline!), tomls + .wallet_pass + the node's"
    echo -e "       wallet_listener_url are re-patched and the listener restarts unlocked."
    echo -e "    4. AFTER: sweep the OLD wallet's balance → the new wallet from a separate"
    echo -e "       box/dir. ${RED:-}If compromised, sweep FIRST — fastest key wins.${RESET:-}"
    echo -e "       ${DIM:-}Coinbase maturity: rewards mined to the old wallet in the last 1440${RESET:-}"
    echo -e "       ${DIM:-}blocks (~1 day) must mature before the sweep completes — keep the${RESET:-}"
    echo -e "       ${DIM:-}archived dir until its balance reads zero.${RESET:-}"
    echo -e "    5. Resume payouts in the admin panel."
    echo ""
    echo -ne "  Payouts paused, ready to replace the wallet? Type ${BOLD:-}REPLACE${RESET:-} to continue: "
    local go; read -r go || true
    [[ "$go" == "REPLACE" ]] || { info "Cancelled — nothing changed."; return 1; }

    pw_listener_stop || true
    echo ""
    if ! pw_setup; then
        error "Wallet setup did not complete — the pool wallet may be DOWN."
        error "  Re-run 5) Set up wallet until it shows [RUNNING], keep payouts paused."
        return 1
    fi

    echo ""
    success "New pool wallet is live (listener up + unlocked, node re-pointed)."
    echo -e "  ${BOLD:-}Remaining manual steps:${RESET:-}"
    echo -e "    · Restart the ${_PW_NODE_NET} node if pw_setup asked for it (coinbase re-point)."
    echo -e "    · Sweep the old (archived) wallet's funds to the new address once mature."
    echo -e "    · Resume payouts in the admin panel and watch the first payout round."
}
