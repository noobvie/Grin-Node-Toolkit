...
# Required — provides nginx_ensure_rate_limit_zones, used by pool_setup_nginx
# to write the rate-limit zones the vhost references. If this file is missing,
# the toolkit is broken; fail loudly rather than silently producing a broken vhost.
...
pool_setup_nginx() {
    ...
    # Rate-limit zones — written to /etc/nginx/conf.d/script07-${POOL_SERVICE}.conf
    # via the shared helper so they live in http context (not inside the vhost file).
