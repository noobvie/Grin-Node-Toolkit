# 059_lib_nginx.sh — Grin Drop nginx configuration
...
drop_create_domain() {
    while true; do
        ...
        echo -e "${BOLD}${CYAN} 059) GRIN DROP — 1) Domain & nginx${RESET}"
        echo -e "${BOLD}${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
        echo ""
...
_drop_write_unified_conf() {
    ...
    local home_src="$TOOLKIT_ROOT/web/059_drop/home"
    ...
    # Migration: Grin Drop moved 052 → 059, so the zone file is renamed. DELETE the
    # old file BEFORE writing the new one — with both present the same zone names
    # are defined twice in the http context, nginx refuses to start and every later
    # `systemctl reload nginx` fails. Never write-then-clean here. (Leaving the old
    # file would also make the helper's duplicate-zone guard skip the new write, so
    # the rename would silently never happen.) nginx is not reloaded between the two,
    # so there is no window where the zones are undefined.
    if [[ -f /etc/nginx/conf.d/script052-drop.conf ]]; then
        rm -f /etc/nginx/conf.d/script052-drop.conf
        info "Removed legacy rate-limit zone file: /etc/nginx/conf.d/script052-drop.conf"
    fi

    # Rate-limit zones — written to /etc/nginx/conf.d/script059-drop.conf
    # via shared helper (lib is sourced by 059_grin_drop.sh before this lib).
    # The helper handles its own fallback if for some reason it's missing.
    if declare -F nginx_ensure_rate_limit_zones &>/dev/null; then
        nginx_ensure_rate_limit_zones "script059-drop" \
            "drop_home:30r/m" \
            "drop_api:10r/m"  \
            "drop_test:5r/m"  \
            "drop_main:5r/m"
...
# Grin Drop — Unified Domain — generated by 059_grin_drop.sh
# Route: / → homepage  /testnet/ → :${test_port}  /mainnet/ → :${main_port}
# Rate-limit zones live in /etc/nginx/conf.d/script059-drop.conf
server {
    listen 80;
    listen [::]:80;
    server_name $domain;
...
# Install nginx + certbot (whichever is missing) + evict apache2 — delegated to
# the shared lib (nginx_install_with_certbot), sourced by 059_grin_drop.sh. The
# wrapper name is kept so the existing call site stays valid; it also gains RHEL
# support that this Debian-only copy lacked.
_drop_nginx_ensure_installed() {
