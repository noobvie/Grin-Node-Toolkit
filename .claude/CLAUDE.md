...
- **Auth:** Admin-only JWT sessions (bcrypt, IP allowlist, 60 min timeout); miners never need accounts.
  ...
  **Admin panel shell — shared LEFT SIDEBAR (reorg 2026-06):** every admin page injects one sidebar via
  `public_html/js/admin-nav.js` (served at `/js/admin-nav.js`, loaded after `theme.js`); the old per-page
  top `<header>`/`nav-links` and the 5-theme chrome switcher were **removed** (the redundant Dashboard+Admin
  pair is gone). The sidebar lists Dashboard/Miners/Payments/Users/Health plus a **Settings** group whose
  sub-links deep-link into `settings.html` tabs via the URL hash (`#access`=Security, `#incentives`, `#seo`,
  …) — Security/Incentives were previously unreachable because nothing linked to settings.html. `settings.html`
  tab switching is now **hash-driven** (`tabFromHash()`/`hashchange`), so `settings.html#access` opens the
  Access Control tab. Layout uses `admin-panel/styles.css` `.sidebar` (fixed) + `body.has-sidebar{padding-left}`;
  `#nav-user` lives in the sidebar footer so `API.guardAdminPage()` still wires username+Logout. Admin chrome
  theme is reduced to a Dark/Light toggle (two `data-theme` buttons that `theme.js init()` auto-wires; storage
  key `admin-theme`) — Matrix/Naruto/Japan dropped from admin chrome.
...
- **Admin operations features (added 2026-06) — payout/ledger/blocks/moderation tooling.** Seven
  ...
  - **Pool blocks explorer** — `GET /api/admin/blocks?status=&limit=&offset=` (secureAdmin) + NEW
    page `admin-panel/blocks.html` + a sidebar "Blocks" link in `public_html/js/admin-nav.js`.
