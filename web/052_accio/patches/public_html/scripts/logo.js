...
class Logo {
	...
	// Public
		...
		constructor(application, message, wakeLock) {
			...
			// ACCIO PATCH — the 3D logo is upstream's MWC mark and we have no
			// Grin model to put in its place, so it never shows.
			//
			// This flag, not the WebGL skip below, is the part that matters:
			// allowShowing() gates on it, and if it stayed true `main` would
			// get class "logo", whose rule hides the SVG wordmark to make room
			// for a canvas that can never paint. The page would then show NO
			// logo at all — worse than upstream's. False here is the same path
			// a browser without WebGL takes, so the CSS keeps the SVG mark.
			//
			// Deleting the subsystem outright (and models/mwc.json with it) is
			// S9's second deletion pass; this is the reversible one-line
			// version that S7 can launch on.
			...
			// ACCIO PATCH — don't acquire a WebGL context at all.
			//
			// Upstream's two getContext calls stood here. Reporting no WebGL
			// takes the branch a browser without it takes: the shader program,
			// the render loop and the fetch of ./models/mwc.json (upstream's
			// MWC mark) never happen, and cleanUp() already guards every
			// teardown on this same value.
			this.gl = Logo.NO_WEBGL;
