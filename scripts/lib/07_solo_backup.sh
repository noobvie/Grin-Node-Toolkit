...
#   · personal_key — ONE shared secret for all product backups; stored b64 in
#     grin_backup.conf (mode 600) so the daily cron can run unattended. Typed
#     by HAND on restore. (Migrated from the old grin_solo_backup.conf.)
...
# This lib keeps only the solo-specific source list + restore steps.
...
sb_load_conf() {
    SB_KEEP="$SB_KEEP_DEFAULT"; SB_HOUR=3; SB_MIN=30
    if [[ -f "$SB_CONF" ]]; then
    ...
    gbe_load_key
    SB_PERSONAL_KEY="$GBE_PERSONAL_KEY"
}
...
sb_prune() {
    gbe_prune_count solo "${1:-$SB_KEEP}" "$SB_BACKUP_DIR"
}
...
sb_backup_now() {
    ...
    # listener touching a file) — that's a warning, not fatal. The engine
    # refuses to encrypt a ZERO-BYTE tar (a truncated/disk-full write).
    ...
    gbe_finalize_archive solo "$tmp_tar" "$SB_BACKUP_DIR" || return 1
    ...
    echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] backup created: $(basename "$GBE_ARCHIVE")" >> "$SB_LOG" 2>/dev/null || true
...
sb_restore() {
    ...
    # Stop any live wallet listener before overwriting wallet_data/ — extracting
    # LMDB over an open listener can corrupt it. Best-effort; the helper exists
    # when sourced by 07_grin_mining_solo.sh (no-op if run standalone).
    if declare -F sw_listener_stop >/dev/null 2>&1; then
        info "Stopping wallet listeners before extraction ..."
        sw_listener_stop mainnet 2>/dev/null || true
        sw_listener_stop testnet 2>/dev/null || true
    fi
...
CONF="$SB_CONF"            # solo schedule/retention
...
maintenance_menu() {
    ...
    while true; do
        ...
        echo -e "  ${DIM}Dir: $SB_BACKUP_DIR · key: ${RESET}$key_state${DIM} · schedule: ${RESET}$sched${DIM} · offsite: ${RESET}$push_state"
        echo ""
        ...
        echo -e "  ${GREEN}7${RESET}) Offsite push (scp)    ${DIM}(auto-copy each archive to a remote server)${RESET}"
        echo ""
