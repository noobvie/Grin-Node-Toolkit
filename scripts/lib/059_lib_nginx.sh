...
_drop_write_unified_conf() {
    ...
    # Rate-limit zones — written to /etc/nginx/conf.d/script052-drop.conf
    # via shared helper (lib is sourced by 052_grin_drop.sh before this lib).
    ...
    if declare -F nginx_ensure_rate_limit_zones &>/dev/null; then
        nginx_ensure_rate_limit_zones "script052-drop" \
            "drop_home:30r/m" \
            "drop_api:10r/m"  \
            "drop_test:5r/m"  \
            "drop_main:5r/m"
...
# Rate-limit zones live in /etc/nginx/conf.d/script052-drop.conf
server {
    listen 80;
    listen [::]:80;
    server_name $domain;
...
# the shared lib (nginx_install_with_certbot), sourced by 052_grin_drop.sh. The
