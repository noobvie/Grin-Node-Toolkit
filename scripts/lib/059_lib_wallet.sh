# 059_lib_wallet.sh — Grin Drop wallet setup + listener management
...
drop_wallet_listener() {
    while true; do
        ...
        # 052_ is the pre-renumber tag — still shown as enabled on an old box.
        crontab -l 2>/dev/null | grep -q "059_watchdog_${DROP_NETWORK}\|052_watchdog_${DROP_NETWORK}" \
...
#!/bin/bash
# Auto-generated by 059_grin_drop.sh — do not edit manually
...
_drop_toggle_watchdog_cron() {
    local tag="059_watchdog_${DROP_NETWORK}"
    # Legacy tag from before Grin Drop moved 052 → 059. Detect AND remove both for
    # one release: a box deployed under the old tag must not get a second watchdog
    # installed alongside it, and "disable" must be able to reach the old entry.
    local legacy_tag="052_watchdog_${DROP_NETWORK}"
    local cur_cron; cur_cron=$(crontab -l 2>/dev/null || true)

    if echo "$cur_cron" | grep -q "$tag\|$legacy_tag"; then
        echo "$cur_cron" | grep -v "$tag" | grep -v "$legacy_tag" | crontab - 2>/dev/null || true
...
#!/bin/bash
# Watchdog — auto-generated by 059_grin_drop.sh
