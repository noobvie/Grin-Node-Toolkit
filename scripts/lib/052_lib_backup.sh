...
#
# Password model (mirrors solo):
#   <personal_key><DDMMYYYY>   e.g.  123abc04072026
#   · personal_key — secret you choose; stored base64 in grin_drop_backup.conf
...
dbk_load_conf() {
    DBK_PERSONAL_KEY=""; DBK_KEEP="$DBK_KEEP_DEFAULT"; DBK_HOUR=3; DBK_MIN=30
    local DBK_PERSONAL_KEY_B64=""
    [[ -f "$DBK_CONF" ]] || return 0
    # shellcheck disable=SC1090
    source "$DBK_CONF" 2>/dev/null || true
    [[ -n "$DBK_PERSONAL_KEY_B64" ]] && DBK_PERSONAL_KEY=$(printf '%s' "$DBK_PERSONAL_KEY_B64" | base64 -d 2>/dev/null || true)
...
dbk_save_conf() {
    ...
    local b64; b64=$(printf '%s' "$DBK_PERSONAL_KEY" | base64 | tr -d '\n')
...
dbk_set_key() {
    ...
    echo -e "  ${RED}${BOLD}   the wallet seed words live INSIDE these archives.${RESET}"
    [[ -n "$DBK_PERSONAL_KEY" ]] && echo -e "  ${DIM}(a key is already set — entering a new one replaces it)${RESET}"
    echo ""
    local k1 k2
    ...
    DBK_PERSONAL_KEY="$k1"; unset k1 k2
    dbk_save_conf
    success "Personal key saved → $DBK_CONF (mode 600). Keep a copy somewhere safe."
}

# Ensure a key exists (prompt to set one if not). rc 1 if still unset.
_dbk_require_key() {
    dbk_load_conf
    if [[ -z "$DBK_PERSONAL_KEY" ]]; then
        warn "No backup personal key set yet — set one now."
        dbk_set_key || return 1
    fi
    [[ -n "$DBK_PERSONAL_KEY" ]]
}
...
dbk_prune() {
    local keep="${1:-$DBK_KEEP}" n
    n=$(ls -1 "$DBK_BACKUP_DIR/${DBK_PREFIX}"*.tar.gz.enc 2>/dev/null | wc -l)
    [[ "$n" -gt "$keep" ]] || return 0
    ls -t "$DBK_BACKUP_DIR/${DBK_PREFIX}"*.tar.gz.enc 2>/dev/null \
...
dbk_backup_now() {
    ...
    local d pass archive tmp_gz
    d=$(dbk_date)
    pass="${DBK_PERSONAL_KEY}${d}"
    archive="$DBK_BACKUP_DIR/${DBK_PREFIX}${d}.tar.gz.enc"
    tmp_gz=$(mktemp "/tmp/grin_drop_bak_XXXXXX.tar.gz") || { error "mktemp failed."; return 1; }

    mkdir -p "$DBK_BACKUP_DIR"; chmod 700 "$DBK_BACKUP_DIR" 2>/dev/null || true

    info "Creating archive..."
    # tar may exit non-zero for a benign "file changed as we read it" (the live
    # listener touching wallet_data) — a warning, not fatal. An EMPTY result is a
    # real failure (disk full / nothing readable): never encrypt that.
    ...
    if ! openssl enc -aes-256-cbc -pbkdf2 -iter 600000 -salt \
            -pass fd:3 -in "$tmp_gz" -out "$archive" 3<<<"$pass" 2>/dev/null; then
        rm -f "$tmp_gz" "$archive"; unset pass
        error "openssl encryption failed."; return 1
    fi
    rm -f "$tmp_gz"; unset pass
    ...
    echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] backup created: $(basename "$archive") ($sz)" >> "$DBK_LOG" 2>/dev/null || true

    dbk_prune "$DBK_KEEP"
    log "[dbk_backup_now] archive=$(basename "$archive")"
}
...
DBK_PERSONAL_KEY_B64=""; DBK_KEEP=$DBK_KEEP_DEFAULT
# shellcheck disable=SC1090
source "\$CONF"
KEY=\$(printf '%s' "\$DBK_PERSONAL_KEY_B64" | base64 -d 2>/dev/null || true)
...
dbk_settings() {
    ...
    while true; do
        ...
        echo -e "  Personal key : $key_state   ${DIM}($DBK_CONF)${RESET}"
...
drop_backup_menu() {
    ...
    while true; do
        ...
        local key_state="${RED}not set${RESET}"; [[ -n "$DBK_PERSONAL_KEY" ]] && key_state="${GREEN}set${RESET}"
        echo -e "  ${DIM}Dir: $DBK_BACKUP_DIR · key: ${RESET}$key_state${DIM} · schedule: ${RESET}$sched"
        echo ""
