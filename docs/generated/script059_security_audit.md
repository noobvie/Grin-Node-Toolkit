...
- **Evidence:** `verifyTurnstile()` returns `true` when no secret is configured
  ([app.js:147-148](../../web/059_drop/server/app.js#L147-L148)) *and* returns `true` on any
  `fetch` exception ([app.js:156-158](../../web/059_drop/server/app.js#L156-L158)).
  `turnstile_secret` defaults to `''` ([config.js:83](../../web/059_drop/server/config.js#L83)).
...
- **Evidence:** `getClientIp()` trusts `cf-connecting-ip` header **first**
  ([app.js:130-132](../../web/059_drop/server/app.js#L130-L132)); the anon endpoint keys its
  rate limit on `hashIp(getClientIp())` ([app.js:461-472](../../web/059_drop/server/app.js#L461-L472)).
  The nginx vhost only overwrites `X-Real-IP` ([059_lib_nginx.sh:395](../../scripts/lib/059_lib_nginx.sh#L395))
...
- **Fix:** Only honour `CF-Connecting-IP` when the request genuinely came from Cloudflare.
  Wire the existing CF `real_ip` helper into `059_lib_nginx.sh` and have the app read the
...
- **Evidence:** `nextClaimIso()` rate-limits by `grin_address`
  ([app.js:161-170](../../web/059_drop/server/app.js#L161-L170)); a claimer can generate
...
- **Evidence:** `drop_name` and `maintenance_message` are interpolated into the maintenance
  503 page without escaping ([app.js:181-184](../../web/059_drop/server/app.js#L181-L184)).
...
- **Evidence:** [app.js:284-285](../../web/059_drop/server/app.js#L284-L285) returns
...
- `wallet_pass_file` (`.temp_<suffix>`) is read on every Owner-API session
  ([wallet.js:60-64](../../web/059_drop/server/wallet.js#L60-L64)). Unavoidable for an
  ...
  [config.js:143](../../web/059_drop/server/config.js#L143)). Same rationale as the pool's
...
- **SQL injection:** none — every query uses `better-sqlite3`/`node:sqlite` prepared statements
  with bound params, incl. the `LIKE` search ([db.js:193-203](../../web/059_drop/server/db.js#L193-L203)).
- **Amount tampering:** claim/anon amounts are clamped server-side to `[0.0001, cap]`
  ([app.js:387-389](../../web/059_drop/server/app.js#L387-L389), 486-488); a user cannot request
...
- **Self-address drain:** claim and invoice both reject the drop wallet's own address
  ([app.js:362-366](../../web/059_drop/server/app.js#L362-L366), 706-710).
- **Input bounds:** `express.json({limit:'16kb'})`, slatepack `≤8192` bytes + format check
  ([app.js:139-143](../../web/059_drop/server/app.js#L139-L143)).
- **Owner API v3 ECDH/AES-256-GCM** implemented correctly (random 12-byte nonce per call, GCM
  auth tag verified on decrypt — [wallet.js:171-214](../../web/059_drop/server/wallet.js#L171-L214)).
- **Exposure:** app binds `127.0.0.1` only ([app.js:955](../../web/059_drop/server/app.js#L955));
...
- **LMDB DoS mitigation:** single serialized background scanner with quiet-window deferral
  keeps a claim flood from fanning out concurrent node scans ([app.js:908-937](../../web/059_drop/server/app.js#L908-L937)).
