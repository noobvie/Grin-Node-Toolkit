					...
					case Consensus.GRIN_WALLET_TYPE:
						...
						if(isMainnet === true) {
							...
							return [
								...
								// ACCIO PATCH — our own node first.
								//
								// Order is not cosmetic here: this list is the
								// connect-and-failover order, so entry 0 is the
								// node that serves almost every session. Ours is
								// the one we can actually keep synced, and the
								// one whose /v2/foreign we publish ourselves
								// (Script 04). The rest are upstream's list,
								// order preserved, kept as failover — a public
								// wallet that can only reach ONE node is a
								// wallet that is down whenever that node is.
						...
						// Otherwise
							...
							return [
								...
								// ACCIO PATCH — our own node first, same
								// reasoning as the mainnet list above.
			...
			return new Promise(function(resolve, reject) {
				...
				// Otherwise
					...
					return JsonRpc.sendRequest(self.getAddresses(Consensus.getNetworkType() === Consensus.MAINNET_NETWORK_TYPE).map(function(address) {
						...
						// Authorization
						"Authorization": "Basic " + Base64.fromUint8Array((new TextEncoder()).encode(self.getUsername(Consensus.getNetworkType() === Consensus.MAINNET_NETWORK_TYPE) + ":" + self.getSecret()))
