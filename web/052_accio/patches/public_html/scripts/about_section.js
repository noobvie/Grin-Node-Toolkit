...
class AboutSection extends Section {
	...
	// Private
		...
		addVersionInformation() {
			...
			// ACCIO PATCH — our source, not upstream's.
			//
			// Upstream branched four ways here (extension / standalone /
			// mobile app / site) because it publishes four artefacts from
			// four repositories. Accio is one repository, so the branch
			// collapses. Set explicitly rather than through the brand map in
			// scripts/language.js, because one of the four URLs it replaced —
			// https://github.com/NicolasFlamel1/mwcwallet.com — is also the
			// target of the MWC Wallet credit in ATTRIBUTIONS, and a map
			// cannot tell "this build's source" from "the work we forked".
			var sourceCode = Language.SOURCE_CODE_URL;
		...
		addTranslationContributionMessage() {
			...
			translationContributorsDisplay.prepend(Language.createTranslatableContainer("<p>", message, [
				[
					// ACCIO PATCH — we have no translations mailbox, so the
					// invitation points at our issue tracker instead. The
					// sentence around it is rewritten to match by the
					// "You can email %1$m" entry in Language.BRAND_SUBSTITUTIONS
					// (English only; other languages keep their own wording
					// around the same link).

					// Text
					Language.CONTACT_URL,
